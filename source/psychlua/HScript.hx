...
import states.MainMenuState.CodeBG;
import states.editors.ChartingState.AttachedFlxText;
import extensions.FlxTextEx;
import backend.FpsTween;
...
class HScript extends SScript
{
	...
	override function preset() {
		...
		#if (!flash && sys)
		set('FlxRuntimeShader', shaders.RuntimeShader);
		#end
		...
		set('RuntimeShader', shaders.RuntimeShader);
		set('addShader',(shader:FlxShader,camera:FlxCamera)->CrossUtil.addShader(shader,camera));
		set('removeShader',(shader:FlxShader,camera:FlxCamera)->CrossUtil.removeShader(shader,camera));
		set('lerp',CrossUtil.betterLerp);
		...
		set('FlxEmitterMode',{
			SQUARE: flixel.effects.particles.FlxEmitter.FlxEmitterMode.SQUARE,
			CIRLCE: flixel.effects.particles.FlxEmitter.FlxEmitterMode.CIRCLE
		});
		...
		if(PlayState.instance == FlxG.state)
		{
			...
			set('makeShader',(shader:String) ->{
				PlayState.instance.initLuaShader(shader);
				return PlayState.instance.createRuntimeShader(shader);
			});
