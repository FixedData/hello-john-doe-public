...
class PlayState extends MusicBeatState
{
	...
	public var healthBounds:FlxBounds<Float> = new FlxBounds(0.,2.);
	...
	public var camGoodBye:FlxCamera; //this was so the gameover would stop acting stupid sorry
	...
	public var defaultCamZoomPlus:Float = 0;
	...
	// Camera utilities
	public var forcedCharFocus:Character = null;
	public var camOffset:Float = 15;
	public var beatsPerZoom:Float = 4;
	var camAngle:Float = 0;

	
	public var robloxHUD:RobloxHUD;

	public var hscriptHUD:Null<HscriptUI>;

	//soon imma make this container
	public var curHUD:Null<Any>;


	public var forcedMiddle:Bool = true;
	...
	override public function create()
	{
		...
		#if debug Paths.clearUnusedMemory(); #end
		...
		camGoodBye = new FlxCamera();
		camHUD.bgColor.alpha = 0;
		camOther.bgColor.alpha = 0;
		camGoodBye.bgColor.alpha = 0;

		FlxG.cameras.add(camHUD, false);
		FlxG.cameras.add(camOther, false);
		FlxG.cameras.add(camGoodBye, false);
		...
		comboGroup = new FlxContainer();
		add(comboGroup);
		noteGroup = new FlxContainer();
		add(noteGroup);
		uiGroup = new FlxContainer();
		add(uiGroup);


		hscriptHUD = HscriptUI.createUI('hud/script');
		if (hscriptHUD != null) {
			add(hscriptHUD);
			hscriptHUD.cameras = [camHUD];
		}
		else {
			robloxHUD = new RobloxHUD();
			add(robloxHUD);
			robloxHUD.cameras = [camHUD];
		...
		Paths.image('logo/${PlayState.SONG.song}');
	...
	//nanabooboo
	function callUIFunc(f:Ihud->Void) 
	{
		if (hscriptHUD != null) f(hscriptHUD);
		else f(robloxHUD);
	}
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000 / playbackRate, function(tmr:FlxTimer)
			{
				...
				notes.forEachAlive(function(note:Note) {
					if(ClientPrefs.data.opponentStrums || note.mustPress)
					{
						...
						if((ClientPrefs.data.middleScroll || forcedMiddle) && !note.mustPress)
							note.alpha *= 0.35;
					}
				});
	...
	public dynamic function updateScore(miss:Bool = false)
	{
		...
		callUIFunc((hud:Ihud)-> hud.onUpdateScore());
	...
	function startSong():Void
	{
		...
		callUIFunc((hud:Ihud)-> hud.onSongStart());

	

		#if DISCORD_ALLOWED
		// Updating Discord Rich Presence (with Time Left)
		if(autoUpdateRPC) DiscordClient.changePresence(detailsText, SONG.song + " (" + storyDifficultyText + ")", '', true, songLength);
		#end
			...
			for (songNotes in section.sectionNotes)
			{
				...
				if(floorSus > 0) {
					for (susNote in 0...floorSus + 1)
					{
						...
						else if(ClientPrefs.data.middleScroll || forcedMiddle)
						{
							sustainNote.x += 310;
	...
	override function openSubState(SubState:FlxSubState)
	{
		...
		if (paused)
		{
			...
			#if VIDEOS_ALLOWED PsychVideoSprite.globalPause(); #end
		}
	...
	override function closeSubState()
	{
		...
		if (paused)
		{
			...
			#if VIDEOS_ALLOWED PsychVideoSprite.globalResume();#end
	...
	function resetRPC(?showTime:Bool = false)
	{
		...
		if (showTime)
			DiscordClient.changePresence(detailsText, SONG.song + " (" + storyDifficultyText + ")", '', true, songLength - Conductor.songPosition - ClientPrefs.data.noteOffset);
		else
	...
	override public function update(elapsed:Float)
	{
		if(!inCutscene && !paused && !freezeCamera) {
			var lerpValue = 2.4 * cameraSpeed * playbackRate;
			FlxG.camera.followLerp = lerpValue;
			//if (!isPixelStage) camGame.scrollAngle = FlxMath.lerp(camGame.scrollAngle,camAngle,1 - Math.exp(-elapsed * (lerpValue)));
		...
		if (healthBounds != null && health > healthBounds.max) health = healthBounds.max;
		...
		if (camZooming)
		{
			FlxG.camera.zoom = FlxMath.lerp(defaultCamZoom + defaultCamZoomPlus, FlxG.camera.zoom, Math.exp(-elapsed * 6.25 * camZoomingDecay * playbackRate));
			camHUD.zoom = FlxMath.lerp(defaultHUDZoom, camHUD.zoom, Math.exp(-elapsed * 6.25 * camZoomingDecay * playbackRate));
		}
		...
		if (FlxG.keys.justPressed.SIX) {cpuControlled = !cpuControlled; botplayTxt.visible = !botplayTxt.visible; botplaySine = 0;}
		#end
	...
	{
		health = value;
		callUIFunc((hud:Ihud)-> hud.onHealthChange());
		return health;
	}
	...
	function openPauseMenu()
	{
		...
		#if DISCORD_ALLOWED
		if(autoUpdateRPC) DiscordClient.changePresence(detailsPausedText, SONG.song + " (" + storyDifficultyText + ")", '');
		#end
	}
	...
	public function triggerEvent(eventName:String, value1:String, value2:String, strumTime:Float) {
		...
		switch(eventName) {
			case 'logo':
				var logo = new FlxSprite().loadGraphic(Paths.image('logo/' + PlayState.SONG.song));
				logo.alpha = 0;
				...
				final time = flValue1 ?? 2.1;

				final delay = flValue2 ?? 0.0;
				FlxTimer.wait(delay,()->{
					FlxTween.tween(logo,{alpha: 1},time,{ease:FlxEase.expoOut, onComplete: Void->{
						FlxTimer.wait(1.2,()->FlxTween.tween(logo, {alpha: 0},time, {ease: FlxEase.cubeOut}));
					}});
					FlxTween.tween(logo, {'scale.x': 0.9, 'scale.y': 0.9},time*2,{ease: FlxEase.quadInOut, startDelay: time/2, onComplete: Void->{remove(logo);logo.destroy();}});
				});
			...
			case 'Change Character':
				...
				callUIFunc((ui:Ihud)->ui.onCharChange());
		...
		callUIFunc((ui:Ihud)->ui.onEventTrigger(eventName,value1,value2,strumTime));
	}
	...
	{
		var cam_DISPLACEMENT:FlxPoint = FlxPoint.get();
		...
		if (char.animation.curAnim != null) 
		{
			if (char.animation.curAnim.curFrame < 3)  {
				switch (char.animation.curAnim.name.substring(4)) {
					case 'UP' | 'UP-alt':
						cam_DISPLACEMENT.y = -camOffset;
					case 'DOWN' | 'DOWN-alt':
						cam_DISPLACEMENT.y = camOffset;
					case 'LEFT' | 'LEFT-alt':
						cam_DISPLACEMENT.x = -camOffset;
						newAngle = 0.5;
	...
	public function endSong()
	{
		...
		if(ret != LuaUtils.Function_Stop && !transitioning)
		{
			...
			Highscore.saveSongData(SONG.song,storyDifficulty,songScore,percent,Highscore.calculateFC(songMisses,percent));
	...
	public var comboGroup:FlxContainer;
	// Stores HUD Objects in a Group
	public var uiGroup:FlxContainer;
	// Stores Note Objects in a Group
	public var noteGroup:FlxContainer;
	...
	override function stepHit()
	{
		...
		callUIFunc((ui:Ihud)->ui.onStepHit());
	}
	...
	override function beatHit()
	{
		...
		callUIFunc((ui:Ihud)->ui.onBeatHit());
	}
	...
	override function sectionHit()
	{
		...
		callUIFunc((ui:Ihud)->ui.onSectionHit());
	}
	...
	public function createRuntimeShader(name:String):RuntimeShader
	{
		if(!ClientPrefs.data.shaders) return new shaders.RuntimeShader();
		...
		return new shaders.RuntimeShader(arr[0], arr[1]);
		#else
	...
	public function snapCamToPos(x:Float = 0,y:Float = 0) {
		camFollow.x = x;
		camFollow.y = y;
		FlxG.camera.snapToTarget();
	}
